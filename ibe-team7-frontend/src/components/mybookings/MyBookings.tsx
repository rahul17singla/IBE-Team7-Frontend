...
export const MyBookings = () => {
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      const response = await axios.get(`${BACKEND_URL}/api/v1/mybookings`, {
            ...
            <div key={booking.bookingMapperId} className="booking">
              <div className="booking-details">
                <div className="booking-info">
                  <span>Booking Id:</span> {booking.bookingMapperId}
                </div>
                <div className="booking-info">
                  <span>Full Name:</span> {booking.firstName} {booking.lastName}
                </div>
                <div className="booking-info">
                  <span>Start Date:</span> {booking.startDate.split("T")[0]}
                </div>
                <div className="booking-info">
                  <span>End Date:</span> {booking.endDate.split("T")[0]}
                </div>
              </div>
              <button
